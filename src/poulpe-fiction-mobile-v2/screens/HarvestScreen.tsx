...
export function HarvestScreen({ bundle, onBackToGerard }: { bundle: HarvestBundle | null; onBackToGerard: () => void }) {
  ...
  return (
    <section className="pf-card" aria-label="Récolte">
      ...
      {harvest.kind === "landing" ? <><div className="pf-visual-preview"><img src={harvest.previewUrl} alt={harvest.title} /></div><div className="pf-caption">{harvest.copy}</div><HarvestActions actions={[{ label: "Copier le texte", onClick: () => copy(harvest.copy) }, { label: "Télécharger HTML", primary: true, onClick: () => download("landing.html", harvest.html, "text/html") }]} /></> : null}
