...
test("prefers a locally stored Publisher API override when present", () => {
  const context = createContext();
  context.localStorage.setItem("poulpe-fiction:publisher-api-url", "https://custom-publisher.example");
  loadRuntimeConfig(context);
  assert.equal(context.PoulpeRuntimeConfig.urls.publisherApi, "https://custom-publisher.example");
});
...
test("removes stale API localStorage overrides without clearing Garden data", () => {
  ...
  context.localStorage.setItem("PUBLISHER_FRONTEND_URL", "http://localhost:4000");
  context.localStorage.setItem("poulpe-fiction:garden-api-url:v1", "http://localhost:5000");
  ...
  assert.equal(context.localStorage.getItem("PUBLISHER_FRONTEND_URL"), null);
  assert.equal(context.localStorage.getItem("poulpe-fiction:garden-api-url:v1"), null);
